...
import com.base.wang.model.BaseEntity;
...
public class EbeanUtil {
   ...
   /* public static  void configQuery(Query query,UriInfo ui){
     ...
     * @param queryParam
     */
    public static void configQueryParam(Query query,QueryParam queryParam){
        Integer maxrows=queryParam.getMaxrows();
        if(maxrows!=null){
            query.setMaxRows(maxrows);
        }
        Integer firstrow=queryParam.getFirstrow();
        if(firstrow!=null){
            query.setFirstRow(firstrow);
        }
        Integer start=queryParam.getStart();
        if(start!=null){
            query.setFirstRow(start);
        }
        ...
        if(filter!=null){
            applyWhere(query,filter);
        }
        String sort=queryParam.getSort();
        if(sort!=null){
            applyWhere(query,sort);
        }
    }
    private static void applyWhere(Query query, String whereJson) {
        try {
            UriOptions.applyWhere(query, whereJson);
